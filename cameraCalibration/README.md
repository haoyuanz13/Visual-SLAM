...
For the data collection, simply capture multiple images of a planar checkerboard pattern (known 3D points). More specifically, the pipeline shows below:
  ...
  * _Images Capture_    
  ...
  • Use 10-20 images for robust calibration. • Disable auto-focus. • Do not change the zoom settings while capturing. • Do not modify your images, e.g., cropping, resizing, or rotating images. • Capture the pattern from diﬀerent angles with a suﬃciently large baseline. • Cover most parts of the images with the pattern. • Do not use a camera lens with large distortion such as a GoPro camera.
