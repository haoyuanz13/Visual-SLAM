...
pos_error_sum = 0; rpy_error_sum = 0; vel_error_sum = 0; 
...
v_x_est = zeros(num_sensor, 1); v_y_est = zeros(num_sensor, 1); v_z_est = zeros(num_sensor, 1);
v_x_gt = zeros(num_sensor, 1); v_y_gt = zeros(num_sensor, 1); v_z_gt = zeros(num_sensor, 1);
...
for i = 1 : num_sensor
  ...
  vel_error = sqrt(sum((vel_cur - vel_vicon) .^ 2));
  vel_error_sum = vel_error_sum + vel_error;

  % display numerical information
  fprintf('[+] Time Stamp: %f || Position Error: %f meter(s) || Euler Angles Error: %f rad(s) || Linear Velocity Error: %f meter(s)/s. \n', sensor_cur.t, pos_error, rpy_error, vel_error);
  ...
  v_x_est(i) = vel_cur(1); v_y_est(i) = vel_cur(2); v_z_est(i) = vel_cur(3);
  v_x_gt(i) = vel_vicon(1); v_y_gt(i) = vel_vicon(2); v_z_gt(i) = vel_vicon(3);

end

pos_error_avg = pos_error_sum / num_sensor;
rpy_error_avg = rpy_error_sum / num_sensor;
vel_error_avg = vel_error_sum / num_sensor;

fprintf('[*] Completed Pose estimation! \n');
fprintf('[+] Average Position Error: %f meter(s) || Average Euler Angles Error: %f. || Average Linear Velocity Error: %f meter(s)/s. \n', pos_error_avg, rpy_error_avg, vel_error_avg);
...
% vel x
ax1 = subplot(3, 1, 1);
plot(ax1, t_axis, v_x_gt, 'r');

hold on
plot(ax1, t_axis, v_x_est, 'b');

legend('Vicon', 'Estimated Result');
title(ax1, 'Linear Velocity (x dimension)')
ylabel(ax1, 'vel_x')

% vel y
ax2 = subplot(3, 1, 2);
plot(ax2, t_axis, v_y_gt, 'r');

hold on
plot(ax2, t_axis, v_y_est, 'b');

legend('Vicon', 'Estimated Result');
title(ax2, 'Linear Velocity (y dimension)')
ylabel(ax2, 'vel_y')

% vel z
ax3 = subplot(3, 1, 3);
plot(ax3, t_axis, v_z_gt, 'r');

hold on
plot(ax3, t_axis, v_z_est, 'b');

legend('Vicon', 'Estimated Result');
title(ax3, 'Linear Velocity (z dimension)')
ylabel(ax3, 'vel_z')
